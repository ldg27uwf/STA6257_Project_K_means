...
[Presentation](https://ldg27uwf.github.io/STA6257_Project_K_means/slides.html#/title-slide)
...
data and the specific clustering technique employed [@berkhin_survey_2006].
...
We are testing 4 methods: 
1. No feature selection, use all data 
2. Hand Selected most relevant features 
3. By Variance Threshold 
4. Lasso
...
-   **Scree Plot**: Look for the elbow point where the plot starts to
    flatten out. This point often indicates the number of components to
    retain.
-   **Cumulative Proportion of Variance Explained**: Choose the number
    of components that explain a satisfactory level of the total
    variance, like 80% or 90%.
-   **Kaiser Criterion**: Retain components with eigenvalues greater
    than 1.
...
-   **Elbow Method**: Look for the "elbow point" where the WSS starts to
    slow down.
-   **Silhouette Method**: Choose the number of clusters that maximize
    the average silhouette width.
-   **Gap Statistic**: Select the number of clusters where the gap
...
cluster.
Clustering is a vital data mining process that assists in data analysis by revealing patterns, trends and data associations within a large dataset by forming clusters of similar data points. This technique is used extensively in both research and business to facilitate decisions and address outcomes. By creating group clusters, it simplifies the data interpretation process and allows the detection of anomalies or outliers. Despite its limitations, such as the need of predetermining the number of clusters and the potential to converge to local optima, k-means is still widely used due to its simplicity and versatility. 

Techniques and modifications continue to evolve to make k-means even more effective. Researchers are addressing these challenges by enhancing the algorithm’s scalability and robustness. These limitations are being addressed by researchers like Arthur, D et al, by improving the first step of k-means to avoid poor clustering due to random initialization. Yang et al propose a new method on selecting the optimal k value by addressing both application-specific and detector-specific factors. The authors proposed the KFC method which offers a parameter free solution with linear time complexity. Yang et al made the KFC method available at www.outliernet.com to an external site to facilitate reproducibility and provide opportunities for others to expand their research. Their research showed that the KFC method outperforms traditional methods and is much more versatile making it useful across many fields. 
Guojun Gan and Michael Kwok-Po Ng propose to improve the k-means clustering algorithm by including outlier detection. The KMOR (k-means with outlier removal) algorithm broadens the classic k-means algorithm by proposing an additional cluster for outliers. The methodology comprises initialization, objective function optimization, iterative process updates, convergence, and parameter control. . The KMOR algorithm’s ability to seamlessly integrate clustering and outlier detection for both synthetic and real datasets showcased its dominance over current methods in terms of accuracy and efficiency. Futuristically, the goals are to investigate diverse ways to control outliers by refining parameter selection and broadening the algorithm for subspace clustering [@gan_k-means_2017].

As future improvements are applied, k-means clustering will continue to enhance its utility across various disciplines. It finds extensive applications across multiple domains including data mining, image processing, bioinformatics, public health, market research, anomaly detection and pattern recognition. Each of these applications demonstrates the ability to provide valuable insights and decision making across a wide range of`fields.
